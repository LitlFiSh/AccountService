...
public class FileGenerator {
    public static HSSFWorkbook generateExcel(OrderApply orderApply, Boolean createSignPath){
        ...
        sheet.addMergedRegion(new CellRangeAddress(n-1, n-1, 0, 8));
        ...
        for(OrderList orderList : orderApply.getOrderLists()){
            ...
            cell.setCellStyle(styleCcc);
            cell = rowList.createCell(l++);
            cell.setCellValue(orderList.getBudgetUnitPrice());
            cell.setCellStyle(styleCcc);
            cell = rowList.createCell(l++);
            cell.setCellValue(orderList.getBudgetTotalPrice());
            cell.setCellStyle(styleCcc);
            cell = rowList.createCell(l++);
        ...
        cellSum = rowSum.createCell(6);
        cellSum.setCellValue(orderApply.getTotal());
        cellSum.setCellStyle(styleCcc);
        //签名
        if(createSignPath){
            ...
            cra = new CellRangeAddress(n-2, n-2, 2, 8);
            sheet.addMergedRegion(cra);
            setBorder(BorderStyle.THIN, cra, sheet);
            ...
            cra = new CellRangeAddress(n-1, n-1, 2, 8);
            sheet.addMergedRegion(cra);
            setBorder(BorderStyle.THIN, cra, sheet);
